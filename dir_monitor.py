    ...
    def get_size_stats(self, size):
        ...
        for f in self.files:
            try:
                ...
                print "status error unable to retrieve file sizes"
                raise    
    ...
    def get_date_stats(self, date):
        ...
        for f in self.files:
            try:
                ...
                print "status error unable to retrieve file modified times"
                raise
            if file_mod > date:
                a_date.append(f)
            elif file_mod < date:
...
if __name__ == '__main__':
    ...
    if not user and not parent_dir:
        if 'HOME' in os.environ:
            ...
            print "status error unable to set home path"
            sys.exit(1)
    ...
    if not os.access(path, os.F_OK):
        print "status error unable to access path",path
        sys.exit(1)
    ...
    print "status success",total,"files found at",path
